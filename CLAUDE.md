...
Positive search inputs are NOT in this file. They live in two user-editable files under `$JOB_DATA_ROOT`: `target-roles.md` (target role titles, title keywords, domains of interest) and `candidate-highlights.md` (quick-reference candidate profile — notable experience, stack, location, certifications). Both are seeded from bundled defaults (`harness-db/harness_db/target-roles.default.md` and `candidate-highlights.default.md`) if missing — `job-seeker` Step 0e does this and never overwrites a tuned copy. Work-type/eligibility/employment come from the resume. `job-seeker` Step 0 synthesizes the resume plus these two files into the generated `candidate-summary.json` that every searcher reads. Exclusions belong in `disqualifiers.yaml`; positive targets belong in `target-roles.md` / `candidate-highlights.md`.
