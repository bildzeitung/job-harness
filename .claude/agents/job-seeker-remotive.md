...
To sanity-check any of the three written files' shape, posting count, and per-field coverage, run `python -m api_search inspect <path>` — do **not** hand-roll a `python3 -c` JSON one-liner for this.
